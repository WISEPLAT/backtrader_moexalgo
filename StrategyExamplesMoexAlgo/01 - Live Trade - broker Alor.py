...
# example of live trading for Alor
from BackTraderAlor.ALStore import ALStore  # Storage Alor
from AlorPy import AlorPy  # Work with Alor OpenAPI V2
from my_config.Config_Alor import Config  # Alor Connection configuration file
...
if __name__ == '__main__':
    ...
    # an example for Alor
    exchange = 'MOEX'  # Stock market
    portfolio = Config.PortfolioStocks  # Stock Market Portfolio
    apProvider = AlorPy(Config.UserName, Config.RefreshToken)  # We connect to the trading account. Login and Refresh Token are taken from the file Config.py
    live_prefix = 'MOEX.'  # prefix for submitting applications in live
    store_alor = ALStore(providers=[dict(provider_name='alor_trade', username=Config.UserName, demo=False, refresh_token=Config.RefreshToken)])  # Alor storage - We connect to the trading account. Login and Refresh Token are taken from the file Config.py
    # - Connect to a broker with our credentials to the selected exchange
    broker = store_alor.getbroker(use_positions=False, boards=Config.Boards, accounts=Config.Accounts)  # broker Alor
